...
> ④ e2e 登录账号 `admin@example.com/password`：user-service 现无该用户（`scripts/seed.py` 已从工作区删除，仅 git 历史 70a326ea 有 5 用户种子；`user_service.db` 空）→ e2e 内注明前置（用户本机需先建账号并跑 `seed_module_permissions.py` 赋权；需 `product:manage` + `order:manage`，admin e2e 先断言 /admin/products 的「商品管理」），账号补建不在本任务范围，收尾向用户说明。
